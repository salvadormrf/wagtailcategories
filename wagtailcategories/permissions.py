...
def user_can_edit_category_type(user, content_type):
...
def user_can_edit_categories(user):
    """ true if user has any permission related to any content type registered
    as a category type """
    category_content_types = get_category_content_types()
    if user.is_active and user.is_superuser:
        # admin can edit settings if any category type exist
        return bool(category_content_types)
    
    permissions = Permission.objects.filter(content_type__in=category_content_types).select_related('content_type')
